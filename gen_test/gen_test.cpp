...
std::unordered_map<int, std::list<std::tuple<int, bool, std::string>>> all_win_styles_2;

bool is_the_same_style(const std::list<std::tuple<int, bool, std::string>>& src, int ghost_num, bool has_eyes, const std::string& fs) {
	for (const auto& iter : src) {
		int num = std::get<0>(iter);
		bool eyes = std::get<1>(iter);
		std::string s = std::get<2>(iter);
		if (num == ghost_num && eyes == has_eyes && s.compare(fs) == 0) {
			return true;
		}
	}
...
std::string format_sequence(std::list<meld_info>& sequences) {
	sequences.sort(compare_meld());
	std::stringstream stream;
	for (const auto& iter_3 : sequences) {
		if (iter_3.z != -1) {
			stream << "[" << iter_3.x << "," << iter_3.y << "," << iter_3.z << "]";
		}
		else {
			stream << "[" << iter_3.x << "," << iter_3.y << "]";
		}
	}
...
void save_data() {
    ...
    for (const auto& iter_1 : all_win_styles_1) {
        ...
        for (const auto& iter_2 : iter_1.second) {
            ...
            fprintf(fp_1, "%d|%s|%d\n", iter_1.first, has_eyes ? "true" : "false", ghost_num);
        }
    }
    fclose(fp_1);
    ...
    for (const auto& iter_1 : all_win_styles_2) {
        ...
        for (const auto& iter_2 : iter_1.second) {
            int ghost_num = std::get<0>(iter_2);
            bool has_eyes = std::get<1>(iter_2);
			std::string fs = std::get<2>(iter_2);
            fprintf(fp_2, "%d|%s|%d|%s\n", iter_1.first, has_eyes ? "true" : "false", ghost_num, fs.c_str());
        }
    }
    fclose(fp_2);
...
bool add_style_2(int cards[], int ghost_num, bool has_eyes, std::list<meld_info>& sequences) {
    int style = 0;
    for (int i=0; i < 7; ++i) {
        style = style * 10 + cards[i];
    ...
    if (iter == all_win_styles_2.end()) {
		std::list<std::tuple<int, bool, std::string>> listGhostEyes;
		listGhostEyes.push_back(std::make_tuple(ghost_num, has_eyes, format_sequence(sequences)));

        all_win_styles_2[style] = listGhostEyes;
    } else {
		std::string fs = format_sequence(sequences);
		if (!is_the_same_style(iter->second, ghost_num, has_eyes, fs)) {
			iter->second.push_back(std::make_tuple(ghost_num, has_eyes, fs));
		}
    }
...
void ghost_derive_style_2(int cards[], int ghost_num, bool has_eyes, std::list<meld_info>& sequences) {
    // 目前来说，最多 8 鬼
    if (ghost_num > 8) {
        return;
    }
    ...
    for (int i=0; i < 7; ++i) {
        ...
        // 如果这种带鬼的牌型已经存在了
        // 那么由它而衍生出来的带更多鬼的牌型也已经添加过了
		if (!add_style_2(cards, ghost_num, has_eyes, sequences)) {
            ++cards[i];
        ...
        // 再多带一张鬼牌
		ghost_derive_style_2(cards, ghost_num + 1, has_eyes, sequences);
...
// 根据无鬼的牌型，衍生出带鬼的牌型
void ghost_derive_style_2(int cards[], bool has_eyes, std::list<meld_info>& sequences) {
    // 如果这个无鬼的牌型已经添加过了
    // 那么对应这个牌型而衍生出来的带鬼的牌型，也都已经添加过了
	if (!add_style_2(cards, 0, has_eyes, sequences)) {
        return;
    }

    // 从带一张鬼牌开始
	ghost_derive_style_2(cards, 1, has_eyes, sequences);
}
...
void gen_cards_2(int cards[], int level, bool has_eyes, std::list<meld_info>& sequences) {
    // 只组成刻子
    for (int i=0; i < 7; ++i) {
        // 每个框中最多只能有 4 个球，不过。。。
        // 在有鬼的情况下，一个框可以最多有 6 个球(2鬼)
        ...
        cards[i] += 3;
		sequences.push_back(meld_info(i, i, i));

        // 球已丢完，更新牌型去吧
		ghost_derive_style_2(cards, has_eyes, sequences);

        // 开始下一轮丢球
        if (level < 4) {
			gen_cards_2(cards, level + 1, has_eyes, sequences);
        }
...
void gen_2_no_eyes() {
	...
	gen_cards_2(cards, 1, false, sequences);
}
...
void gen_2_with_eyes() {
    ...
    for (int i=0; i < 7; ++i) {
        cards[i] = 2;
		sequences.push_back(meld_info(i, i));

        // 单独只有将牌的时候，碰了 4 次的时候，就单吊将牌
		ghost_derive_style_2(cards, true, sequences);

        // 开始 4 次丢球之旅
		gen_cards_2(cards, 1, true, sequences);
