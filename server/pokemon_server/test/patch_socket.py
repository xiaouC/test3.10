...
class socket(object):
    ...
    def recv(self, size):
        if self.replaying:
            return self.replay_records.pop(0)
...
def start_record(socks):
    for sock in socks:
        sock.start_record()

def start_replay(socks):
    for sock in socks:
        sock.start_replay()

def run_with_recording(socks, func):
    for sock in socks:
        sock.start_record()
    func()

def run_with_replay(socks, func):
    for sock in socks:
        sock.start_replay()
    func()
