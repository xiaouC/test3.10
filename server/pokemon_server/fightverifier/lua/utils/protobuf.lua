...
function _R_meta:__index(key)
    local decoder = decode_type_cache[self._CType][key]
    if decoder then
        local v = decoder(self, key)
        self[key] = v
        return v
    end
end
...
local function expand(tbl)
	...
	tbl[1] , tbl[2] = nil , nil
	assert(c._decode(P, decode_message_cb , tbl , typename, buffer), c._last_error(P))
	setmetatable(tbl , default_table(typename))
end

function forceExpand(v)
	local t = type(v)
	if t ~= "table" and t ~= "userdata" then return end

	local m = getmetatable(v)
	if m == decode_message_mt then
		expand(v)
	end
...
function ppairs(proto)
    return getmetatable(proto).__pairs(proto)
end

function get_optional(obj, name)
    local a = obj[name]
