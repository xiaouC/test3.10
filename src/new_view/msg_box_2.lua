-- ./app/platform/room/new_view/msg_box_2.lua

local popup_base = require 'app.platform.common.popup_base'
local MsgBox2 = class('MsgBox2', popup_base)
function MsgBox2:ctor(scene_instance, args, show_anim_func, hide_anim_func)
    self.csb_file = 'hall_res/common/msg_box_2.csb'

    popup_base.ctor(self, scene_instance, args, show_anim_func, hide_anim_func)
end

function MsgBox2:initViews()
    ...
    local function __create_label_widget__(text)
        local text_label = cc.Label:createWithSystemFont(text, '黑体', 16, cc.size(700, 0))
